...
export default function Employees({ handleNext }) {
  ...
  const noEmployees =
    "Parece que no hay empleados disponibles en este momento, vuelve a intentarlo más tarde.";
  ...
  const {
    ...
    queryFn: () => {
      return axiosInstance.get(`/employees?Page=${1}&Limit=${50}`);
    },
  });

  const employees = getEmployees?.data?.data
    .filter((e) => e.status !== "Desactivado")
    .map((item, index) => {
      return (
        <li
          key={index}
